...
void Screen::SetCursorPosition(uint8_t x, uint8_t y)
{
	...
	if (x >= CAMERA_TEXT_COLUMNS)
	{
		x = CAMERA_TEXT_COLUMNS - 1;
	}

	if (y >= CAMERA_TEXT_ROWS)
	{
		y = CAMERA_TEXT_ROWS - 1;
	}
...
void Screen::PrintChar(char c, uint16_t color)
{
	...
	case '\0': //null
		break;
	case '\n': //line feed
		if (this->_cursor_y < CAMERA_TEXT_ROWS - 1)
		{
...
void Screen::CursorNext()
{
	uint8_t x = this->_cursor_x;
	uint8_t y = this->_cursor_y;
	if (x < CAMERA_TEXT_COLUMNS - 1)
	{
		x++;
	}
	else
	{
		if (y < CAMERA_TEXT_ROWS - 1)
		{
			x = 0;
			y++;
		}
