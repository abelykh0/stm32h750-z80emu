...
public:
  ...
  struct RasterInfo {
    ...
    // Number of 240MHz timer ticks per pixel.  The default value for this in
    // the current mode is given to the rasterizer; the rasterizer may alter
    // it in the result to render fewer, wider-held samples (reduced
    // horizontal resolution) if it can't produce a full line's worth of
    // pixels in time -- see timing.h/timing.cc's 1024x768/1920x1080 modes.
